        ...
        "sources": [
            {
                ...
                "verification_actor": "fact-reviewer",
    ...
    claims: dict[str, object] = {
        "claims": [
            ...
            },
            {
                ...
                "claim": "사실에서 추론했다.",
                "classification": "INFERENCE",
                "canonical_claim_hash": statement_hash("사실에서 추론했다."),
                "evidence_source_ids": [],
                "basis_fact_ids": ["FACT-01"],
                "evidence_scope": "검증된 사실에서 도출",
                "confidence": "MEDIUM",
                "presented_as_fact": False,
            },
            {
                ...
                "claim": "장면을 재구성했다.",
                "classification": "DRAMATIZATION",
                "canonical_claim_hash": statement_hash("장면을 재구성했다."),
                "evidence_source_ids": [],
                "basis_fact_ids": [],
                "evidence_scope": "각색 표시",
                "confidence": "LOW",
                "presented_as_fact": False,
            },
        ]
...
def test_complete_fact_evidence_chain_passes() -> None:
    ...
    assert validate_fact_integrity("VERIFIED_TRUE_CASE", facts, sources, claims, ledger) == []
...
def test_fact_evidence_bundle_passes_schema() -> None:
    ...
    schema = load_json_object(ROOT / "STANDARD" / "schemas" / "fact_evidence.schema.json")
...
def test_missing_evidence_and_disguised_dramatization_fail() -> None:
    ...
    issues = validate_fact_integrity("VERIFIED_TRUE_CASE", changed_facts, sources, claims, ledger)
...
def test_fact_claim_classification_mismatch_fails() -> None:
    ...
    issues = validate_fact_integrity("VERIFIED_TRUE_CASE", facts, sources, claims, ledger)
    assert "FACT_CLAIM_CLASSIFICATION_MISMATCH" in {issue["code"] for issue in issues}
...
def test_inference_cycle_fails() -> None:
    """Inference Basis Graph의 순환 참조는 차단해야 한다."""
    facts, sources, claims, ledger = make_fact_artifacts()
    fact_records = facts["facts"]
    claim_records = claims["claims"]
    assert isinstance(fact_records, list)
    assert isinstance(claim_records, list)
    inference = fact_records[1]
    inference_claim = claim_records[1]
    assert isinstance(inference, dict)
    assert isinstance(inference_claim, dict)
    inference["basis_fact_ids"] = ["FACT-02"]
    inference_claim["basis_fact_ids"] = ["FACT-02"]

    issues = validate_fact_integrity("VERIFIED_TRUE_CASE", facts, sources, claims, ledger)
    assert "INFERENCE_CYCLE" in {issue["code"] for issue in issues}


def test_dramatization_cannot_be_inference_basis() -> None:
    """Dramatization은 Inference의 근거가 될 수 없다."""
    facts, sources, claims, ledger = make_fact_artifacts()
    fact_records = facts["facts"]
    claim_records = claims["claims"]
    assert isinstance(fact_records, list)
    ...
    assert isinstance(inference, dict)
    assert isinstance(inference_claim, dict)
    inference["basis_fact_ids"] = ["FACT-03"]
    inference_claim["basis_fact_ids"] = ["FACT-03"]

    issues = validate_fact_integrity("VERIFIED_TRUE_CASE", facts, sources, claims, ledger)
    assert "DRAMATIZATION_USED_AS_BASIS" in {issue["code"] for issue in issues}


def test_source_snapshot_or_archive_locator_is_required() -> None:
    """Source는 Snapshot Hash 또는 Archive Locator 하나를 가져야 한다."""
    facts, sources, claims, ledger = make_fact_artifacts()
    source_records = sources["sources"]
    assert isinstance(source_records, list)
    source = source_records[0]
    assert isinstance(source, dict)
    source.pop("source_snapshot_sha256")

    issues = validate_fact_integrity("VERIFIED_TRUE_CASE", facts, sources, claims, ledger)
    assert "SOURCE_PROVENANCE_MISSING" in {issue["code"] for issue in issues}


def test_duplicate_canonical_claim_fails() -> None:
    """한 Fact에 Canonical Claim이 둘 이상이면 실패해야 한다."""
    facts, sources, claims, ledger = make_fact_artifacts()
    claim_records = claims["claims"]
    assert isinstance(claim_records, list)
    claim_records.append(deepcopy(claim_records[0]))

    issues = validate_fact_integrity("VERIFIED_TRUE_CASE", facts, sources, claims, ledger)
    assert "DUPLICATE_CANONICAL_CLAIM" in {issue["code"] for issue in issues}
