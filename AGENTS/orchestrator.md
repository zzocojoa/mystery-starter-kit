...
Production Standard, Compatibility Contract, Channel DNA, Project State를 결합하고 GATE-00부터 GATE-13까지 순서대로 실행한다. Gate가 실패하면 다음 Agent를 호출하지 않고 `BLOCKED` 상태와 구체적 Issue를 기록한다.

## 입력과 출력

- 입력: Compatibility Report, Dependency Graph, Agent Manifest, 각 QA Report
- 출력: Production Config, Project State, 통합 Validation Report, Production Package
...
5. 일반 프로젝트는 `AUTO_CONTINUE`로 진행하고 Human Override가 필요한 예외만 정지한다.
